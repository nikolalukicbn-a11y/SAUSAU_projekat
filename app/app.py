"""
KORAK 5: STREAMLIT DEPLOYMENT v5 - Bagging ensemble (10 models, K-voting)
...
@st.cache_resource
def load_artifacts():
    bagged = joblib.load(os.path.join(MODELS_DIR, "model3_bagging", "bagged_models.pkl"))
    preproc = joblib.load(os.path.join(MODELS_DIR, "model3_bagging", "preprocessor.pkl"))
    K_val = joblib.load(os.path.join(MODELS_DIR, "model3_bagging", "bagging_k.pkl"))
    return bagged, preproc, K_val
...
bagged_models, preprocessor, model_K = load_artifacts()
...
def run_prediction(circuit, rider, team, bike, year, sequence):
    ...
    probs = []
    for name, model in bagged_models:
        p = float(model.predict_proba(input_processed)[0, 1])
        label = "xgb" if "xgb" in name else "lr"
        probs.append((label, p))
    # Return list of (model_type, probability) tuples
...
if "bag_thresh" not in st.session_state:
    st.session_state.bag_thresh = 0.35
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### Parametri bagging-a")

    new_K = st.slider("K (min. glasova za DNF)", 1, 10, st.session_state.K, 1,
                      help="DNF ako bar K od 10 bagged modela predvidi DNF")
    if new_K != st.session_state.K:
        st.session_state.K = new_K
        if st.session_state.last_result is not None:
            st.session_state.run_flag = True

    new_thr = st.slider("Threshold po modelu", 0.0, 1.0, st.session_state.bag_thresh, 0.01,
                        help="Prag verovatnoce za svaki od 10 modela")
    if new_thr != st.session_state.bag_thresh:
        st.session_state.bag_thresh = new_thr
        if st.session_state.last_result is not None:
            st.session_state.run_flag = True

    st.caption(f"K={st.session_state.K}/10 modela mora da predvidi DNF")

    st.markdown("---")
    with st.expander("O modelu"):
        st.markdown(f"""
        - **Arhitektura**: 5x XGBoost + 5x LogisticRegression (bootstrap)
        - **Feature-a**: 16 (TargetEncoded, kalibrisano Platt scaling-om)
        - **Voting**: DNF ako K={st.session_state.K} od 10 modela glasa DNF
        - **Recall DNF**: 97.5% | **FN (test)**: 6/244 (K=2)
...
if st.session_state.run_flag:
    probs, features = run_prediction(
        p["circuit"], p["rider"], p["team"], p["bike"],
        p["year"], p["sequence"]
    )
    # K-voting: count how many models predict DNF
    votes = sum(1 for _, prob in probs if prob >= st.session_state.bag_thresh)
    is_dnf = votes >= st.session_state.K
    max_proba = max(p for _, p in probs)
    xgb_votes = sum(1 for label, prob in probs if "xgb" in label and prob >= st.session_state.bag_thresh)
    lr_votes = sum(1 for label, prob in probs if "lr" in label and prob >= st.session_state.bag_thresh)
    st.session_state.last_result = {
        "probs": probs, "features": features,
        "is_dnf": is_dnf, "votes": votes,
        "max_proba": max_proba,
        "xgb_votes": xgb_votes, "lr_votes": lr_votes,
    }
    st.session_state.run_flag = False
...
if st.session_state.last_result is not None:
    ...
    voter_info = f"({votes}/10 modela: DNF, K={st.session_state.K})"
    ...
    st.plotly_chart(create_gauge(max_proba, st.session_state.bag_thresh), use_container_width=True)

    st.markdown("---")
    st.markdown("### Detalji predikcije (K-voting)")
    ...
    with d3:
        st.metric(f"Glassovi za DNF", f"{votes}/10 (K={st.session_state.K})")
    with d4:
...
<div class="custom-footer">
    ...
    Bagging · K={st.session_state.K}/10 · thr={st.session_state.bag_thresh:.0%} · FN(test)=6/244
</div>
""", unsafe_allow_html=True)
